...
__global__ void generateRayFromCamera(Camera cam, int iter, int traceDepth, PathSegment* pathSegments)
{
    ...
    float jitter_y = y;
      ...
      jitter_x = (float)x + u01(rng);
      jitter_y = (float)y + u01(rng);
    #endif

		segment.ray.direction = glm::normalize(cam.view
			- cam.right * cam.pixelLength.x * ((float)jitter_x - (float)cam.resolution.x * 0.5f)
			- cam.up * cam.pixelLength.y * ((float)jitter_y - (float)cam.resolution.y * 0.5f)
			);
	...
	)
{
	...
	if (path_index < num_paths)
	{
		...
		for (int i = 0; i < geoms_size; i++)
		{
			...
			{
        #if MOTION_BLUR
          float k = sin(float(iter)/1.f) / 16.0f;
          geom.transform = glm::translate(geom.transformCopy, glm::vec3(0.f, k, 0.f));
...
)
{
  ...
  if (!COMPACT && pathSegments[index].remainingBounces == 0) return;
  ...
  if (intersection.t > 0.0f) {
    ...
    // TODO: replace this! you should be able to start with basically a one-liner
      ...
      glm::vec3 intersectionPoint = getPointOnRay(pathSegments[index].ray, intersection.t);
...
void pathtrace(uchar4 *pbo, int frame, int iter) {
  ...
  bool iterationComplete = false;
    ...
    dim3 numblocksPathSegmentTracing = (num_paths + blockSize1d - 1) / blockSize1d;
	  // tracing
    #if CACHING
      //compute intersections for initial bounce of first iteration and for all non-initial bounces
      ...
      //cache initial bounce intersections of first iteration
      if (iter == 1 && depth == 0)
        cudaMemcpy(dev_intersections_cache, dev_intersections, pixelcount * sizeof(ShadeableIntersection), cudaMemcpyDeviceToDevice);
      //use cached intersections for every initial bounce in remaining iterations
    ...
    #if COMPACT
      PathSegment *path_end = thrust::partition(thrust::device, dev_paths, dev_paths + num_paths, pathEnd());
      num_paths = path_end - dev_paths;
      iterationComplete = depth >= traceDepth || path_end == 0;
    #else
