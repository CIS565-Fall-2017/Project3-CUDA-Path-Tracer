...
struct compmat {
	__host__ __device__ bool operator()(const ShadeableIntersection& ma, const ShadeableIntersection& mb) {
		return ma.materialId < mb.materialId;
	}
};
...
)
{
	...
	if (path_index < num_paths)
	{
		...
		else
		{
			...
			intersections[path_index].intersect = intersect_point; //new variable added to ShadeableIntersection
		}
	}
}
...
void pathtrace(uchar4 *pbo, int frame, int iter) {
	...
	int numpathtmp = num_paths; //store for later use
